...
with bruto AS (
    ...
    SELECT
        ...
        COALESCE(liquido.valor, 0) AS salario
    FROM
        empregado
        LEFT JOIN liquido
...
FROM
    emp_liq
    LEFT JOIN avg_div
        ON emp_liq.lotacao_div = avg_div.lotacao_div
WHERE
    emp_liq.salario > avg_div.valor AND
    emp_liq.salario >= 8000
ORDER BY
    emp_liq.lotacao_div;
