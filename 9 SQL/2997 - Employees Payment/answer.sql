...
SELECT
    ...
    COALESCE(ROUND(bruto.valor, 2), 0) AS "Salario Bruto",
    COALESCE(ROUND(custo.valor, 2), 0) AS "Total Desconto",
    COALESCE(ROUND(bruto.valor, 2), 0) - COALESCE(ROUND(custo.valor, 2), 0) AS "Salario Liquidoaws"
FROM
    empregado
    LEFT JOIN departamento
        ON empregado.lotacao = departamento.cod_dep
    LEFT JOIN bruto
        ON empregado.matr = bruto.matr
    LEFT JOIN custo
        ON empregado.matr = custo.matr
ORDER BY
