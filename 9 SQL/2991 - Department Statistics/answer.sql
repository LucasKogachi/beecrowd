...
with bruto AS (
    ...
    SELECT
        empregado.lotacao,
        COUNT(*) AS num_emp,
        ROUND(AVG(COALESCE(liquido.valor, 0)), 2) AS media,
        ROUND(MAX(COALESCE(liquido.valor, 0)), 2) AS maior,
        ROUND(MIN(COALESCE(liquido.valor, 0)), 2) AS menor
    FROM
        empregado
        LEFT JOIN liquido
...
SELECT
    ...
    dep_liq.num_emp   AS "Numero de Empregados",
    dep_liq.media     AS "Media Salarial",
    dep_liq.maior     AS "Maior Salario",
    CASE
        WHEN dep_liq.menor = 0 THEN 0
        ELSE TRUNC(dep_liq.menor, 2)
    END AS "Menor Salario"
FROM
    departamento
    INNER JOIN dep_liq
        ON departamento.cod_dep = dep_liq.lotacao
ORDER BY
    "Media Salarial" DESC;
